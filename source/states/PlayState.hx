...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['Your death', 0.2], //From 0% to 19%
		['DEATH IS NEAR', 0.4], //From 20% to 39%
		['Do not do ANY worser', 0.5], //From 40% to 49%
		['Average score', 0.6], //From 50% to 59%
		['Close.', 0.69], //From 60% to 68%
		['OH COME ON', 0.7], //Why tf you get this? silly goose
		['Better', 0.8], //From 70% to 79%
		['Even Better', 0.9], //From 80% to 89%
		['BEST', 1], //From 90% to 99%
	...
	public dynamic function fullComboFunction()
	{
		...
		if(songMisses == 0)
		{
			if (shits > 0) ratingFC = 'BadNM C.';
			else if (bads > 0) ratingFC = 'GoodNM B.';
			else if (goods > 0) ratingFC = 'GreatNM A.';
			else if (sicks > 0) ratingFC = 'SpecialNM A+.';
		}
		else {
			if (songMisses < 10) ratingFC = 'Missed10 D.';
			else ratingFC = 'MoreMissed F.';
		}
	}
